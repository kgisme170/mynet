...
RUN apt-get update \
    ...
    && apt-get -y install --fix-missing gradle python python-dev ruby scons swig tcl tcl-dev uuid-dev vim wget \
    && echo "Done installing standard software" \
    ...
    && apt-get update \
    && apt-get install sbt \
    && echo "Done installing scala and sbt" \
    && echo "deb [arch=amd64] http://storage.googleapis.com/bazel-apt stable jdk1.8" | tee /etc/apt/sources.list.d/bazel.list \
    && curl https://bazel.build/bazel-release.pub.gpg | apt-key add - \
    && apt-get update \
    && apt-get -y install --fix-missing bazel \
    && echo "Done installing google bazel" \
